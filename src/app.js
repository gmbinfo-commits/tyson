...
function load() {
  ...
  try {
    ...
    const hydrated = {
      ...
      managers: Array.isArray(parsed.managers) ? parsed.managers : fallback.managers,
      admins: Array.isArray(parsed.admins) ? parsed.admins : fallback.admins,
      patients: Array.isArray(parsed.patients) ? parsed.patients : fallback.patients,
    ...
    if (hydrated.role && !roles.includes(hydrated.role)) hydrated.role = null;
    if (!hydrated.user || !hydrated.role) {
      hydrated.role = null;
      hydrated.user = null;
      hydrated.page = null;
    }

    if (!hydrated.selectedCaseId || !hydrated.cases.some((c) => c.id === hydrated.selectedCaseId)) {
      hydrated.selectedCaseId = hydrated.cases[0]?.id || fallback.selectedCaseId;
    }
